...
from api_utils import upload_document, list_documents, delete_document, upload_doc, delete_documents
...
def display_sidebar():
    ...
    if st.sidebar.button("Upload sites"):
            with st.spinner("Uploading..."):
                upload_response = upload_doc()
                if upload_response:
                    st.sidebar.success(f"Sites uploaded successfully with ID.")
                    st.session_state.documents = list_documents()  # Refresh the list after upload
    if st.sidebar.button("Delete Documents"):
            with st.spinner("Deleting..."):
                delete_response = delete_documents()
                if delete_response:
                    st.sidebar.success(f"Documents deleted successfully.")
                    st.session_state.documents = list_documents()  # Refresh the list after deletion
                else:
                    st.sidebar.error(f"Failed to delete documents.")
