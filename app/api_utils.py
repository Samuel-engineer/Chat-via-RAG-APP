...
def upload_doc():
    print("Uploading file...")
    try:
        response = requests.post("http://localhost:8000/upload-web-doc")
        if response.status_code == 200:
            return response.json()
        else:
            st.error(f"Failed to upload file. Error: {response.status_code} - {response.text}")
...
def delete_documents():
    ...
    try:
        response = requests.post("http://localhost:8000/deleteAll-doc", headers=headers)
        if response.status_code == 200:
            return response.json()
        else:
            st.error(f"Failed to delete documents. Error: {response.status_code} - {response.text}")
            return None
    except Exception as e:
        st.error(f"An error occurred while deleting the documents: {str(e)}")
        return None